...
- [ ] Task 1.2: 將 `pnpm test` 切為 Vitest 主入口，保留舊驗證為 legacy smoke（若仍需要），並在 `package.json` 宣告 `packageManager` 使用 pnpm（Green）
- [ ] Task 1.3: 導入 Vite tooling skeleton、多 entrypoint build、`tsconfig` 與 `src/tooling/shared/*` 基礎 module（Green）
- [ ] Task 1.4: 建立 module fixture 目錄與測試資料約定（Green）
- [ ] Task 1.5: 定義 `SOURCE_ROOT`、`PUBLISH_ROOT`、`DIST_ROOT` 等 shared paths constants，並測試 source/dist 執行環境解析一致（Green）
- [ ] Task 1.6: 更新 `vitest.config` include，確保新 `src/**/*.test.ts` 與既有測試可並行（Refactor）
...
- [ ] Task 2.1: 以真實 agents 的複製版建立 `agent-transpiler` fixture input 與 expected golden output（Red）
...
- [ ] Task 2.10: 驗證 root `dist/` 只包含 Vite tooling；平台 agent output 只生成到 `.publish/ddd-workflow/dist/{gemini,opencode,codex}/agents`（Refactor）
...
- [ ] Task 3.1: 撰寫 publish tree allowlist/denylist contract 測試，確認 `_runtime/` 不會原樣 publish（Red）
- [ ] Task 3.2: 實作 `sync-publish-tree`，只同步 publishable content，並保留 managed checkout 的 `.git`（Green）
...
- [ ] Task 3.7: 實作 build-publish 注入 generated `bin/*.mjs`、self-contained skill runtime 實體檔，並產生 `.publish/ddd-workflow/dist/{gemini,opencode,codex}/agents`（Green）
- [ ] Task 3.8: 驗證 build 不複製外層 `docs/`、`reference/`、`.opencode/`、`dist/`、local-only 檔案（Refactor）
- [ ] Task 3.9: 驗證 `.publish/ddd-workflow` 不包含任何 symlink（Refactor）
...
- [ ] Task 4.2: 搬移 `ddd-workflow/agents`、`references`、`config`、`policies`、`.claude-plugin`、`gemini-extension.json`（Green）
- [ ] Task 4.3: 將 package-level runtime scripts 搬入 `src/ddd-workflow/scripts/{claude,opencode,shared}` namespace（Green）
...
- [ ] Task 4.6: 加入 `.publish/` ignore 與 publish repo 自身 `dist/` ignore（Green）
- [ ] Task 4.7: 移除舊 `ddd-workflow/` source root，避免與 `src/ddd-workflow/` 並存（Green）
- [ ] Task 4.8: 更新測試中所有舊 `ddd-workflow/` source path 為新 source / publish contract（Refactor）
...
- [ ] Task 5.1: 定義 deploy action contract：copy / overwrite-generated / copy-if-missing-config / skip / dry-run event schema；不建立 symlink（Red）
...
- [ ] Task 6.1: 撰寫 package scripts command contract 測試，鎖定 `test -> build -> test:pack -> deploy-local` gate 與 `deploy:dry-run` 無副作用（Red）
- [ ] Task 6.2: 收斂 `package.json` scripts 為新 pipeline；`pnpm deploy` 預設實際部署，`pnpm deploy:dry-run` 只列出動作（Green）
...
- [ ] Task 7.3: 更新專案操作文件，完全移除 subtree 主流程說明；歷史脈絡只保留在 works.md（Green）
