...
日常手改內容放在 `src/ddd-workflow/`；`pnpm run build` 會產生 `.publish/ddd-workflow/`，再由 repo-local `skills` CLI 與 publish package 內的 `bin/` entrypoints 供本機或公開 repo 使用。
...
4. 執行 `pnpm run test:pack`，確認 repo-local `skills` CLI 可辨識所有 `ddd.*` skills。
