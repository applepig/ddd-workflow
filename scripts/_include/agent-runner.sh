#!/usr/bin/env bash
# agent-runner.sh — shared runner for DDD skill-local agent entrypoints.
#
# Modes:
...
RUNNER_MODE="$(infer_mode_from_invocation "$self_name")"
...
get_xreview_adapter_dir() {
  local xreview_script_dir="$1"
  printf '%s\n' "$xreview_script_dir/adapters"
}
...
xreview_log_path() {
  local run_id="$1"
  local spec="$2"
  printf '/tmp/xreview-%s-%s.log\n' "$run_id" "$(slug_of "$spec")"
}

xreview_final_path() {
  local run_id="$1"
  local spec="$2"
  printf '/tmp/xreview-%s-%s.final.txt\n' "$run_id" "$(slug_of "$spec")"
}

xreview_status_path_from_log() {
  local log="$1"
  printf '%s.status\n' "${log%.log}"
}
...
emit_fail_event() {
  ...
  local result_key="$4"
  local result="$5"
  echo "FAIL $id exit_code=$exit_code log=$log $result_key=$result"
}
...
script_dir="$(get_xreview_script_dir "$runner_file")"
adapter_dir="$(get_xreview_adapter_dir "$script_dir")"
...
cleanup() {
  ...
  cleanup_tmp_file "$_tmp_prompt_file"
}
...
valid_specs=()
for spec in "${specs[@]}"; do
  log="$(xreview_log_path "$runid" "$spec")"
  final="$(xreview_final_path "$runid" "$spec")"
...
EOF
    ...
    echo "2" > "$(xreview_status_path_from_log "$log")"
    emit_start_event "$spec" "$log"
    emit_fail_event "$spec" "2" "$log" "final" "$final"
    continue
  fi
...
if [[ "$mode" == "blocking" ]]; then
  ...
  for spec in "${specs[@]}"; do
    log="$(xreview_log_path "$runid" "$spec")"
    final="$(xreview_final_path "$runid" "$spec")"
    status_file="$(xreview_status_path_from_log "$log")"
