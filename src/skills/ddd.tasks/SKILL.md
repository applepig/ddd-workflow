...
> 驗證方式：`pnpm vitest run server/routes/auth/` 全過；`curl -X POST localhost:3000/auth/login -d '{"email":"test@t.com","password":"pass"}' | jq .token` 回傳 JWT
...
> 驗證方式：`pnpm vitest run components/auth/` 全過
- [ ] Task 2.3: 撰寫登入表單元件測試 (Red)
- [ ] Task 2.4: 實作登入表單元件 (Green)

### 🔗 匯合點
> 合併 [A]、[B] 分支後，在主線執行整合驗證。
> 驗證方式：`pnpm vitest run tests/integration/auth/` 全過；啟動 dev server 後手動登入流程正常
...
3. **撰寫 tasks.md**
   ...
   **✅ 好的拆解**——測試先行、平行標記清楚、每條工作線有完整上下文卡片：
   ...
   > 驗證方式：`pnpm vitest run server/models/ server/services/session/`
   ...
   **[A] Backend API** — `isolation: worktree`
   ...
   > 介面契約：POST /auth/login → LoginResponse { token, user }
   > 驗證方式：`pnpm vitest run server/routes/auth/` 全過
   ...
   **[B] Frontend Form** — `isolation: worktree`
   ...
   ### 🔗 匯合點
   > 驗證方式：`pnpm vitest run tests/integration/auth/` 全過
