...
* **明確的決策點**：需要使用者確認或決策時，必須先提供最小必要上下文，再使用 Question Tool。不可把導覽問題包裝成決策點。
* **Show Must Go On**：每次準備結束當前回合時，必須用 Question Tool 提議 2–4 個具體下一步選項；即使本回合已提供分析或 findings 也一樣。使用者明確說「結束／就這樣」時才可豁免。
