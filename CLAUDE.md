...
install.sh             # 安裝腳本：symlink AGENTS.md + skills 到各 agent 設定目錄
src/skills/            # DDD 工作流 skills，每個 skill 一個子目錄
  ddd.<name>/
...
- **install.sh 清理策略**：每次安裝會先移除目標 skills 目錄中所有 `ddd*` 項目，再重新建立個別 symlink，確保不留殘餘
...
- 新增 skill 時在 `src/skills/` 下建立 `ddd.<name>/SKILL.md`，遵循現有 YAML frontmatter 格式
- Skill 的 `name` 欄位用 `DDD.PascalCase`，`description` 須同時包含中文說明和英文 trigger phrases
- 新增 skill 目錄後需重新執行 `./install.sh` 建立 symlink
