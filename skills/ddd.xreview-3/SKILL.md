...
name: DDD.Xreview3
description: >
  Cross review v3——使用多種模型進行 cross review，採用分段約束 prompt 提升 finding 品質。
  Claude subagent 固定使用，外部模型透過 OpenCode CLI 呼叫，具體模型清單見 AGENTS.md。
  Use when the user says "review code", "cross review", "let's review",
  "check my changes", "review this sprint", or invokes "/DDD.xreview3".
  Use after development work to get independent code review from multiple
  AI models before committing or pushing.
---

# DDD:xreview-3 — Cross Review v3

使用多種獨立模型交叉審查程式碼變更。Claude subagent 固定參與，外部模型透過 OpenCode CLI 呼叫——具體使用哪些模型見 AGENTS.md 的「Cross Review 模型設定」。
...
不需要預先讀取內容——每個 reviewer 會自行蒐集。

### 2. 偵測環境與讀取模型設定

```bash
which opencode 2>/dev/null
runner="$HOME/.claude/skills/ddd.xreview-3/scripts/xreview-runner.sh"
ls "$runner" 2>/dev/null
```

- **兩者都存在**：讀取 AGENTS.md「Cross Review 模型設定」表格，確認要派出的外部模型清單
- **OpenCode 不可用**：僅 Claude subagent，在報告中明確標示「因 OpenCode 未安裝，僅使用單一 reviewer」

### 3. 組裝 Review Prompt 並寫入暫存檔
...
```bash
bash ~/.claude/skills/ddd.xreview-3/scripts/xreview-runner.sh \
  "$review_prompt_file" <model-id>
```

以 `Bash({ command: ..., run_in_background: true })` 執行。對表格中每一行未註解的模型都派一個。

> `xreview-runner.sh` 是刻意保持精簡的 shell proxy：只包 timeout（預設 600 秒），並直接轉送 OpenCode 的 stdout/stderr。它不對 review 內容做語意判斷；但若 OpenCode 自己在 stderr 明確吐出 error marker，runner 會把那次執行視為失敗，補一行 `XREVIEW_ERROR` summary。
>
> OpenCode 的 `ddd.xreviewer` agent 需預先部署到 `~/.config/opencode/agents/ddd.xreviewer.md`，設定 read-only permission 和 `steps` 上限。詳見 `references/opencode-reviewer.md`。
...
- OpenCode 非零 exit code：輸出 `XREVIEW_ERROR: opencode exited with code ...`
- OpenCode 自己透過 stderr 回傳的明確錯誤：原樣保留在輸出中，並轉成失敗
...
- **OpenCode CLI**：需安裝並設定好認證（Copilot credentials 或 API key）
- **OpenCode reviewer agent**：需部署到 `~/.config/opencode/agents/ddd.xreviewer.md`（見 `references/opencode-reviewer.md`）
...
- 若未安裝 OpenCode，skill 會退化為僅 Claude subagent 的單方 review
